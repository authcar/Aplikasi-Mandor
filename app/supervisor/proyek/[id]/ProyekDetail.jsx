  ...
  return (
    <main className="p-4 pb-8">
      ...
      <header className="mb-4">
        <h1 className="text-xl font-bold">{proyek.nama}</h1>
        <p className="text-sm text-gray-500">{proyek.lokasi}</p>
      </header>

      <div className="mb-5">
        <label className="mb-1 block font-medium">Mandor Penanggung Jawab</label>
        <select
          value={mandorId}
      ...
      <div className="mb-4 space-y-2">
        {tukang.filter((t) => t.is_active).map((t) => (
          <div key={t.id} className="flex items-center justify-between rounded-xl border-2 border-gray-200 bg-white p-3">
            <div>
              <p className="font-semibold">{t.nama}</p>
              <p className="text-sm text-gray-500">{t.jabatan} · {rupiah(t.upah_harian)}/hari</p>
            </div>
            <button onClick={() => hapusTukang(t.id)} className="text-sm font-medium text-red-500">
              Hapus
            </button>
          </div>
        ))}
        {tukang.filter((t) => t.is_active).length === 0 && (
          <p className="text-sm text-gray-400">Belum ada tukang. Tambahkan di bawah.</p>
        )}
      </div>

      <form onSubmit={tambahTukang} className="space-y-3 rounded-2xl border-2 border-dashed border-gray-300 bg-white p-4">
        <p className="font-semibold">Tambah Tukang</p>
        <input
          value={nama}
        ...
        <button
          disabled={busy}
          className="w-full rounded-xl bg-brand p-3 text-lg font-bold text-white disabled:opacity-60"
        >
