...
export default async function DashboardFinance() {
  ...
  // Finance melihat semua proyek aktif (tidak difilter supervisor_id)
  ...
  const [
    ...
    // Pengajuan KASBON baru dari Supervisor yang belum dilihat Finance.
    supabase.from("keuangan").select("id", { count: "exact", head: true }).eq("jenis", "KASBON").eq("dibaca_master", false),
    // Laporan harian baru dari Supervisor yang belum dilihat Finance.
    ...
    // "belum lapor" di bawah, jadi Finance gak perlu cek manual satu-satu.
    proyekIds.length
  ...
  return (
    <main className="flex min-h-dvh flex-col p-4 gap-3">
      <header className="flex shrink-0 items-center justify-between">
        <div>
          <p className="text-xs font-medium text-brand">Finance</p>
      ...
      {(proyekBelumAbsen.length > 0 || proyekBelumLapor.length > 0 || proyekTanpaMandor.length > 0) && (
        <div className="card shrink-0 divide-y divide-gray-100">
          ...
          {proyekTanpaMandor.length > 0 && (
            <Link href="/finance/proyek-tanpa-mandor" className="flex items-center justify-between gap-2 px-3.5 py-3 active:bg-gray-50">
      ...
      <div className="shrink-0 grid grid-cols-2 gap-2">
        ...
        <Link href="/finance/perbaikan" className="relative card-tap flex flex-col items-center justify-center gap-1 p-3">
      ...
      <ProyekSayaCard proyek={proyek || []} basePath="/finance/proyek" />
    </main>
  );
}
