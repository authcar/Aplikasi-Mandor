...
// Riwayat Checklist Perbaikan (read-only) — Finance memantau semua proyek,
...
export default async function PerbaikanFinancePage() {
  const { supabase } = await getSessionProfile();

  const { data: rows } = await supabase
    .from("checklist_perbaikan")
    .select(
      "id, no, uraian, foto_url, foto_bukti_url, video_url, video_bukti_url, foto_drive_file_id, foto_bukti_drive_file_id, video_drive_file_id, video_bukti_drive_file_id, periode, status, catatan_tolak, created_at, selesai_at, proyek(nama), creator:created_by(name)"
    )
